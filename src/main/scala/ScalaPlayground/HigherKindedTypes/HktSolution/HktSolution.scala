...
@main def run(): Unit =
  ...
  val listPincher = new PinchEnabler[List]:
    def transform[A, B](arr: List[A], f: A => B): List[B] = arr.map(f)
    def pinch[A](arr: List[A], f: A => Unit): List[A]     = { arr.foreach(f); arr }

  doIt(List("hello", "world"))(_.toUpperCase)(using listPincher)
